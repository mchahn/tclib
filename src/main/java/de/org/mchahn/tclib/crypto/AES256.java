...
public class AES256 extends BlockCipher {
    ...
    private static final int shift(int r, int shift) {
    ...
    private static final int FFmulX(int x) {
        return (((x & m2) << 1) ^ (((x & m1) >>> 7) * m3));
    }

    private static final int inv_mcol(int x) {
        int f2 = FFmulX(x);
        int f4 = FFmulX(f2);
        int f8 = FFmulX(f4);
        int f9 = x ^ f8;
    ...
    private static final int subWord(int x) {
        return (S[ x         & 255] & 255         |
              ((S[(x >>>  8) & 255] & 255) <<  8) |
              ((S[(x >>> 16) & 255] & 255) << 16) |
                S[ x >>> 24       ]        << 24);  // !optimized
    ...
    private static final int unpack(byte[] bytes, int index) {
    ...
    private static final void pack(int val, byte[] bytes, int index) {
    ...
    private static final byte[] S  = BinUtils.longsToBytesLE(S_packed);
    private static final byte[] Si = BinUtils.longsToBytesLE(Si_packed);
